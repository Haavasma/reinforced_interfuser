...
class RoutePlanner:
    ...
    def _get_hazard(self) -> Tuple[float, float]:
        ...
        vehicle_distance = self._is_vehicle_hazard(vehicle_list)

        # check for the state of the traffic lights
        light_state = self._is_light_red_us_style(lights_list)

        return light_state, vehicle_distance

    def _is_vehicle_hazard(self, vehicle_list) -> float:
        """
    ...
    def _is_light_red_us_style(self, lights_list) -> float:
        """
        ...
        if self._target_waypoint is not None:
            if self._target_waypoint.is_intersection:
                ...
                if sel_traffic_light is not None:
                    ...
                    if (
                        self._last_traffic_light.state
                        == carla.libcarla.TrafficLightState.Red
                    ):
                        return sel_magnitude
                else:
                    self._last_traffic_light = None
