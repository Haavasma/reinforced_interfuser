...
class CarlaEnv(gym.Env):
    ...
    def __init__(
        ...
        observation_space_dict = {
            ...
            #     low=0,
            #     high=255,
            #     shape=(32 * pixels_per_meter, 32 * pixels_per_meter, 3),
            #     dtype=np.float32,
            # ),
            "state": spaces.Box(
                state_range_min,
    ...
    def step(self, action):
        ...
        print("time passed: ", time.time() - self.start_time)
    ...
    def render(self, _=None) -> np.ndarray:
        ...
        rgb_image = self.rgb[0]
        # lidar_bev = self.lidar_bev[0]

        # segmentation_pred = self.seg_decoder(self.image_features_grid)

        # new_lidar = np.zeros((32 * pixels_per_meter, 32 * pixels_per_meter, 3))
    ...
    def _init_renderer(self):
        ...
        self.im2 = ax2.imshow(np.zeros(((32, 32, 3))))
    ...
    def _get_obs(self):
        ...
        # self.lidar_bev = self.prepare_lidar(point_cloud)
        ...
        # lidar = np.zeros((32 * pixels_per_meter, 32 * pixels_per_meter, 3))
        # lidar[:, :, 0] = lidar_bev[0, :, :]
        # lidar[:, :, 1] = lidar_bev[1, :, :]
        # lidar[:, :, 2] = lidar_bev[2, :, :]

        rgb = np.zeros((160, 704, 3))
        rgb[:, :, 0] = rgb_image[0, :, :]
        rgb[:, :, 1] = rgb_image[1, :, :]
        rgb[:, :, 2] = rgb_image[2, :, :]
