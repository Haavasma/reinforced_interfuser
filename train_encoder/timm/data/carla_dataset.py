...
def get_route_frames(root: str, index_file: str):
    route_frames = []
...
class CarlaMVDetDataset(BaseIODataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment_prob > 0:
            ...
            rgb_left_image = Image.fromarray(
                self.augmenter(image=np.array(rgb_left_image))
            )
            rgb_right_image = Image.fromarray(
                self.augmenter(image=np.array(rgb_right_image))
            )
        ...
        affordances = self._load_npy(
            os.path.join(route_dir, "affordances/%04d.npy" % frame_id)
        )
