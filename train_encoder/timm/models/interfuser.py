...
class Interfuser(nn.Module):
    def __init__(
        ...
        if use_different_backbone:
            if rgb_backbone_name == "levit":
                self.rgb_backbone = levit_128(
                    pretrained=True,
                    in_chans=in_chans,
                    features_only=True,
                    out_indices=[4],
            ...
            if lidar_backbone_name == "mobile":
                self.lidar_backbone = MobileNetV3(
                    pretrained=False,
                    in_chans=in_chans,
                    features_only=True,
                    out_indices=[4],
    ...
    def forward_encoding(self, x):
        ...
        return self.encoder(features, mask=self.attn_mask)
