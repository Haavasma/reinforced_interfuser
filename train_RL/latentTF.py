...
class GPT(nn.Module):
    ...
    def forward(self, image_tensor, lidar_tensor, velocity):
        ...
        if self.use_velocity == True:
            ...
            x = self.drop(
                self.pos_emb + token_embeddings + velocity_embeddings.unsqueeze(1)
            )  # (B, an * T, C)
        else:
    ...
    def __init__(self, architecture, normalize=True):
        ...
        if architecture.startswith(
            ...
            self.features.global_pool.norm = nn.LayerNorm(
                (512, 1, 1), _tmp.eps, _tmp.elementwise_affine
            )
...
class LidarEncoder(nn.Module):
    ...
    def __init__(self, architecture, in_channels=2):
        ...
        if architecture.startswith(
            ...
            self._model.act1 = (
                nn.Sequential()
            )  # Conv NeXt does not use an activation function after the stem
            self._model.maxpool = nn.Sequential()
