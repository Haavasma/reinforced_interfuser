...
def main():
    ...
    parser.add_argument(
        "--epochs", type=int, default=41, help="Number of train epochs."
    )
    ...
    parser.add_argument(
        "--root_dir",
        type=str,
        default=r"/mnt/qb/geiger/kchitta31/datasets/carla/pami_v1_dataset_23_11",
    ...
    parser.add_argument(
        "--val_every", type=int, default=5, help="At which epoch frequency to validate."
    )
    parser.add_argument(
        "--no_bev_loss",
        type=int,
    ...
    model = LidarCenterNet(
        ...
        device,
        args.backbone,
        args.image_architecture,
        args.lidar_architecture,
    ...
    if (bool(args.zero_redundancy_optimizer) == True) and (parallel == True):
        ...
        optimizer = ZeroRedundancyOptimizer(
            model.parameters(), optimizer_class=optim.AdamW, lr=args.lr
    ...
    train_set = CARLA_Data(
        root=config.train_data, config=config, shared_dict=shared_dict
    )
    ...
    if not (args.load_file is None):
        ...
        optimizer.load_state_dict(
            torch.load(
                args.load_file.replace("model_", "optimizer_"),
    ...
    for epoch in range(trainer.cur_epoch, args.epochs):
        ...
        if (
            (epoch == args.schedule_reduce_epoch_01)
            or (epoch == args.schedule_reduce_epoch_02)
        ) and (args.schedule == 1):
...
class Engine(object):
    ...
    def __init__(
        ...
        self.detailed_weights = {
            key: detailed_losses_weights[idx]
            for idx, key in enumerate(self.detailed_losses)
        }
    ...
    def load_data_compute_loss(self, data):
        ...
        if (
            (self.args.backbone == "transFuser")
            or (self.args.backbone == "late_fusion")
            or (self.args.backbone == "latentTF")
        ):
            losses = self.model(
                rgb,
                lidar,
    ...
    def log_losses(self, loss_epoch, detailed_losses_epoch, num_batches, prefix=""):
        ...
        if self.rank == 0:
            ...
            self.writer.add_scalar(
                prefix + "loss_total", aggregated_total_loss, self.cur_epoch
            )
    ...
    def save(self):
        ...
        torch.save(
            ...
            os.path.join(self.args.logdir, "model_%d.pth" % self.cur_epoch),
        )
        torch.save(
            self.optimizer.state_dict(),
            os.path.join(self.args.logdir, "optimizer_%d.pth" % self.cur_epoch),
        )
