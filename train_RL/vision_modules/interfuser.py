...
class InterFuserVisionModule(VisionModule):
    ...
    def __call__(self, world_state: WorldState, postprocess=True):
        ...
        self.pred_waypoints = (
            waypoint_probs[0].detach().cpu().numpy()[0]
        )  # [0][0] is for the mean, excluding variance in the first value of the batch
        self.is_junction = (
        ...
        if self.use_target_feature:
            ...
            # Returning waypoint_probability feature
            return waypoint_feature.squeeze(0).cpu().numpy()
