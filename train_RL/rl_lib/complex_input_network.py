...
import numpy as np
from timm.models import MobileNetV3, MobileNetV3Features, resnet18d
from timm.models.efficientnet import (
    efficientnet_b0,
    efficientnet_lite0,
    efficientnetv2_s,
)
from timm.models.mobilenetv3 import mobilenetv3_rw
...
class ConditionalComplexInputNetwork(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        ...
        for i, component in enumerate(self.flattened_input_space):
            ...
            if len(component.shape) == 3 and isinstance(component, Box):
                ...
                # features = 256
                # cnn = mobilenetv3_rw(pretrained=True, in_chans=3, features_only=False)
                # cnn.classifier = nn.Linear(
                #     in_features=cnn.classifier.in_features, out_features=256
                # )
                #
                # self.cnns[i] = cnn
                #
                ...
                # print("NUM OUTPUTS: ", self.cnns[i].num_outputs)
                concat_size += self.cnns[i].num_outputs
                # concat_size += features
        ...
        self.conditional_post_fc_stack = nn.ModuleList(
            [
                ...
                for _ in range(
                    self.model_config["custom_model_config"]["num_conditional_inputs"]
                )
            ]
        )
        ...
        if num_outputs:
            ...
            self.logits_layer = SlimFC(
                in_size=self.conditional_post_fc_stack[0].num_outputs,
    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        command = input_dict["obs"]["command"]
        ...
        if (
            ...
            raise ValueError(
                "The number of conditional inputs must match the number of "
        ...
        command_idx = torch.argmax(command, dim=-1)

        outputs = []
        for i, network in enumerate(self.conditional_post_fc_stack):
            output, _ = network(SampleBatch({SampleBatch.OBS: out}))
            mask = (command_idx == i).unsqueeze(-1)
            output = output * mask

            outputs.append(output)

        out = torch.sum(torch.stack(outputs), dim=0)

        # out, _ = selected_network(SampleBatch({SampleBatch.OBS: out}))
