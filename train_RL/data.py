            ...
            for route in routes:
                ...
                for seq in range(2, num_seq - self.pred_len - self.seq_len - 2):
                    ...
                    for idx in range(self.seq_len):
                        ...
                        bev.append(
                            route_dir / "topdown" / ("encoded_%04d.png" % (seq + idx))
                        )
                        depth.append(route_dir / "depth" / ("%04d.png" % (seq + idx)))
                        semantic.append(
                            route_dir / "semantics" / ("%04d.png" % (seq + idx))
                        )
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            if (
                ...
                lidars_i = np.load(str(lidars[i], encoding="utf-8"), allow_pickle=True)[
                    1
                ]  # [...,:3] # lidar: XYZI
                if backbone == "geometric_fusion":
                    lidars_raw_i = np.load(
                        str(lidars[i], encoding="utf-8"), allow_pickle=True
                    )[1][
                        ..., :3
                    ]  # lidar: XYZI
                else:
                ...
                bev_array = cv2.imread(
                    str(bevs[i], encoding="utf-8"), cv2.IMREAD_UNCHANGED
                )
                ...
                if self.multitask:
                    depths_i = cv2.imread(
                        str(depths[i], encoding="utf-8"), cv2.IMREAD_COLOR
                    )
                    if depths_i is None:
                        print("Error loading file: ", str(depths[i], encoding="utf-8"))
                    depths_i = scale_image_cv2(
                        cv2.cvtColor(depths_i, cv2.COLOR_BGR2RGB), self.scale
                    )
                    ...
                    if semantics_i is None:
                        print(
                            "Error loading file: ", str(semantics[i], encoding="utf-8")
                        )
                ...
                if not self.data_cache is None:
                    ...
                    compressed_bevs = (
                        io.BytesIO()
                    )  # bev has 2 channels which does not work with png compression so we use generic numpy in memory compression
        ...
        if self.multitask:
            ...
            depths_i = get_depth(
                crop_image_cv2(
                    depths_i, crop=self.img_resolution, crop_shift=crop_shift
                )
            )

            semantics_i = loaded_semantics[self.seq_len - 1]
            semantics_i = self.converter[
                crop_seg(semantics_i, crop=self.img_resolution, crop_shift=crop_shift)
            ]
        ...
        for i in range(self.seq_len):
            ...
            lidar = align(
                lidar, measurements[i], measurements[self.seq_len - 1], degree=degree
            )
...
def align(lidar_0, measurements_0, measurements_1, degree=0):
    ...
    transform_0_to_1 = (
        Tr_vehicle_to_lidar @ np.linalg.inv(matrix_1) @ matrix_0 @ Tr_lidar_to_vehicle
    )
...
def load_crop_bev_npy(bev_array, degree):
    ...
    cropped_image = np.concatenate(
        (
            np.zeros_like(cropped_image[:1]),
            cropped_image[:1],
            cropped_image[:1] + cropped_image[1:2],
        ),
        axis=0,
    )
...
def correspondences_at_one_scale(
    valid_bev_points, valid_cam_points, lidar_x, lidar_y, camera_x, camera_y, scale
):
    """
    ...
    for i in range(valid_bev_points.shape[0]):
        tmp_bev[
            valid_bev_points[i][0] // scale, valid_bev_points[i][1] // scale
        ].append(valid_cam_points[i] // scale)
        tmp_cam[
            valid_cam_points[i][0] // scale, valid_cam_points[i][1] // scale
        ].append(valid_bev_points[i] // scale)
    ...
    for i in range(lidar_x):
        for j in range(lidar_y):
            ...
            if len(cam_to_bev_points) > 5:
                cam_to_bev_proj_locs[i, j] = np.array(
                    random.sample(cam_to_bev_points, 5)
                )
    ...
    for i in range(camera_x):
        for j in range(camera_y):
            ...
            if len(bev_to_cam_points) > 5:
                bev_to_cam_proj_locs[i, j] = np.array(
                    random.sample(bev_to_cam_points, 5)
                )
...
def lidar_bev_cam_correspondences(
    world, lidar_vis=None, image_vis=None, step=None, debug=False
):
    """
    ...
    lidar_height = 256
    lidar_meters_x = (
        lidar_width / pixels_per_meter
    ) / 2  # Divided by two because the LiDAR is in the center of the image
    ...
    fov_height = 2.0 * np.arctan(
        (img_height / img_width) * np.tan(0.5 * np.radians(fov_width))
    )
    ...
    world[
        ...
        -1
    )  # flip x axis, so that the positive direction points towards right. new coordinate system: x right, y forward, z up
    ...
    result_center = result_center[
        np.logical_and(result_center[..., 0] > 0, result_center[..., 0] < img_width)
    ]
    result_center = result_center[
        np.logical_and(result_center[..., 1] > 0, result_center[..., 1] < img_height)
    ]
    ...
    result_left = result_left[
        np.logical_and(result_left[..., 0] > 0, result_left[..., 0] < img_width)
    ]
    result_left = result_left[
        np.logical_and(result_left[..., 1] > 0, result_left[..., 1] < img_height)
    ]
    ...
    result_right = result_right[
        np.logical_and(result_right[..., 0] > 0, result_right[..., 0] < img_width)
    ]
    result_right = result_right[
        np.logical_and(result_right[..., 1] > 0, result_right[..., 1] < img_height)
    ]

    # We only use half of the left image, so we cut the unneccessary points
    result_right_shifted = result_right[
        result_right[..., 0] < (img_width / 2.0)
    ]  # Cut of right part, it's not used.
    result_right_shifted[..., 0] = (
        result_right_shifted[..., 0] + (img_width / 2.0) + img_width
    )

    # Combine the three images into one
    results_total = np.concatenate(
        (result_left_shifted, result_center_shifted, result_right_shifted), axis=0
    )
    ...
    for i in range(results_total.shape[0]):
        ...
        bev_y = (
            int(lidar[lidar_index][1] * pixels_per_meter) - (lidar_height - 1)
        ) * -1
    ...
    if debug == True:
        ...
        plt.imshow(vis_bev)
        plt.savefig(
            r"/home/hiwi/save folder/Visualizations/2/bev_lidar_{}.png".format(step),
            bbox_inches="tight",
        )
        plt.close()
        plt.imshow(vis_original_image)
        plt.savefig(
            r"/home/hiwi/save folder/Visualizations/2/image_with_lidar_{}.png".format(
                step
            ),
            bbox_inches="tight",
        )
