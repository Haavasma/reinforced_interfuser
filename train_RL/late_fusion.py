...
class LateFusionBackbone(nn.Module):
    ...
    def __init__(
        ...
        if image_architecture.startswith("convnext"):
            self.norm_after_pool_img = nn.LayerNorm(
                (self.config.perception_output_features,), eps=1e-06
            )
        else:
            self.norm_after_pool_img = nn.Sequential()

        if lidar_architecture.startswith("convnext"):
            self.norm_after_pool_lidar = nn.LayerNorm(
                (self.config.perception_output_features,), eps=1e-06
            )
        else:
        ...
        if (
            self.image_encoder.features.num_features
            != self.config.perception_output_features
        ):
            self.reduce_channels_conv_image = nn.Conv2d(
                self.image_encoder.features.num_features,
                self.config.perception_output_features,
                (1, 1),
            )
        else:
            self.reduce_channels_conv_image = (
                nn.Sequential()
            )  # For backwards compatibility. ResNet models were trained without this.
        if (
            self.image_encoder.features.num_features
            != self.config.perception_output_features
        ):
            self.reduce_channels_conv_lidar = nn.Conv2d(
                self.lidar_encoder._model.num_features,
                self.config.perception_output_features,
                (1, 1),
            )
        else:
...
class LidarEncoder(nn.Module):
    ...
    def __init__(self, architecture, in_channels=2):
        ...
        self._model = timm.create_model(
            architecture, pretrained=False, in_chans=in_channels
        )
