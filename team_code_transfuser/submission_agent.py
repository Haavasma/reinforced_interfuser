...
class HybridAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        if ('use_target_point_image' in self.args):
            self.config.use_target_point_image = bool(
                self.args['use_target_point_image'])
        ...
        if ('backbone' in self.args):
            # Options 'geometric_fusion', 'transFuser', 'late_fusion', 'latentTF'
            self.backbone = self.args['backbone']
        else:
            # Options 'geometric_fusion', 'transFuser', 'late_fusion', 'latentTF'
            self.backbone = 'transFuser'

        # Stores the last x updated gps signals.
        self.gps_buffer = deque(maxlen=self.config.gps_buffer_max_len)
        # Bicycle model used for de-noising the GPS
        self.ego_model = EgoModel(dt=self.config.carla_frame_rate)

        self.bb_buffer = deque(maxlen=1)
        # x, y, z coordinates of the LiDAR position.
        self.lidar_pos = self.config.lidar_pos
        # Iou threshold used for Non Maximum suppression on the Bounding Box predictions.
        ...
        for file in os.listdir(path_to_conf_file):
            if file.endswith(".pth"):
                ...
                net = LidarCenterNet(self.config, 'cuda', self.backbone,
                ...
                state_dict = torch.load(os.path.join(
                    path_to_conf_file, file), map_location='cuda:0')
                # Removes the .module coming from the Distributed Training. Remove this if you want to evaluate a model trained without DDP.
        ...
        # Test time data augmentation. Unused we only augment by 0 degree.
        self.aug_degrees = [0]
    ...
    def _init(self):
        self._route_planner = RoutePlanner(
            self.config.route_planner_min_distance, self.config.route_planner_max_distance)
    ...
    def tick(self, input_data):
        ...
        for pos in ['left', 'front', 'right']:
            print(pos)
            rgb_cam = 'rgb_' + pos
            print(input_data[rgb_cam][1].shape)
            rgb_pos = cv2.cvtColor(
                input_data[rgb_cam][1][:, :, :3], cv2.COLOR_BGR2RGB)
            rgb_pos = self.scale_crop(Image.fromarray(rgb_pos), self.config.scale, self.config.img_width,
                                      self.config.img_width, self.config.img_resolution[0], self.config.img_resolution[0])
            print(rgb_pos.shape)
            rgb.append(rgb_pos)
        ...
        local_command_point = np.array(
            [next_wp[0]-denoised_pos[0], next_wp[1]-denoised_pos[1]])
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.step % self.config.action_repeat == 1:
            self.update_gps_buffer(
                self.control, tick_data['compass'], tick_data['speed'])
            return self.control
        ...
        if (self.backbone == 'latentTF'):  # Image only method
            lidar_bev = torch.zeros((1, 2, self.config.lidar_resolution_width, self.config.lidar_resolution_height)).to(
                'cuda', dtype=torch.float32)  # Dummy data
        else:
            # prepare LiDAR input
            if (self.config.use_point_pillars == True):
                lidar_cloud = deepcopy(input_data['lidar'][1])
                lidar_cloud[:, 1] *= -1  # invert
                lidar_bev = [torch.tensor(lidar_cloud).to(
                    'cuda', dtype=torch.float32)]
                num_points = [torch.tensor(len(lidar_cloud)).to(
                    'cuda', dtype=torch.int32)]
            else:
                lidar_bev = self.prepare_lidar(tick_data)

        # prepare goal location input
        target_point_image, target_point = self.prepare_goal_location(
            tick_data)

        # prepare velocity input
        gt_velocity = torch.FloatTensor([tick_data['speed']]).to(
            'cuda', dtype=torch.float32)  # used by controller
            ...
            for i in range(self.model_count):
                ...
                if (self.backbone == 'transFuser'):
                    ...
                    pred_wp, _ = self.nets[i].forward_ego(
                        image, lidar_bev, target_point, target_point_image, velocity, num_points=num_points)
                    ...
                    bev_points.append(torch.from_numpy(
                        curr_bev_points).unsqueeze(0))
                    cam_points.append(torch.from_numpy(
                        curr_cam_points).unsqueeze(0))

                    bev_points = bev_points[0].long().to(
                        'cuda', dtype=torch.int64)
                    cam_points = cam_points[0].long().to(
                        'cuda', dtype=torch.int64)
                    pred_wp, _ = self.nets[i].forward_ego(
                        image, lidar_bev, target_point, target_point_image, velocity, bev_points, cam_points, num_points=num_points)
        ...
        bbs_vehicle_coordinate_system = self.non_maximum_suppression(
            bounding_boxes, self.iou_treshold_nms)

        self.bb_buffer.append(bbs_vehicle_coordinate_system)
        self.pred_wp = torch.stack(pred_wps, dim=0).mean(
            dim=0)  # Average the predictions from the ensembles
        ...
        self.pred_wp = torch.median(torch.from_numpy(self.pred_wp).to(
            'cuda', dtype=torch.float32), dim=0, keepdims=True)[0]
        ...
        if (self.backbone == 'latentTF'):
            ...
            # z-axis
            safety_box = safety_box[safety_box[..., 2]
                                    > self.config.safety_box_z_min]
            safety_box = safety_box[safety_box[..., 2]
                                    < self.config.safety_box_z_max]

            # y-axis
            safety_box = safety_box[safety_box[..., 1]
                                    > self.config.safety_box_y_min]
            safety_box = safety_box[safety_box[..., 1]
                                    < self.config.safety_box_y_max]

            # x-axis
            safety_box = safety_box[safety_box[..., 0]
                                    > self.config.safety_box_x_min]
            safety_box = safety_box[safety_box[..., 0]
                                    < self.config.safety_box_x_max]
        ...
        if self.use_lidar_safe_check:
            ...
            # We only use the saftey box when unblocking
            if ((emergency_stop == True) and (is_stuck == True)):
                print(
                    "Detected object directly in front of the vehicle. Stopping. Step:", self.step)
                control.steer = float(steer)
    ...
    def bb_detected_in_front_of_vehicle(self, ego_speed):
        ...
        for bb in self.bb_buffer[-1]:
            ...
            bb_extent_x = 0.5 * \
                np.sqrt((bb[3, 0] - bb[0, 0]) ** 2 +
                        (bb[3, 1] - bb[0, 1]) ** 2)
            bb_extent_y = 0.5 * \
                np.sqrt((bb[0, 0] - bb[1, 0]) ** 2 +
                        (bb[0, 1] - bb[1, 1]) ** 2)
            ...
            bb_local.rotation = carla.Rotation(
                0.0, np.rad2deg(bb_orientation).item(), 0.0)
    ...
    def non_maximum_suppression(self, bounding_boxes, iou_treshhold):
        ...
        bounding_boxes = np.array(
            list(itertools.chain.from_iterable(bounding_boxes)), dtype=np.object)
    ...
    def update_gps_buffer(self, control, theta, speed):
        ...
        action = np.array(
            np.stack([control.steer, control.throttle, control.brake], axis=-1))

        # Update gps locations
        for i in range(len(self.gps_buffer)):
            loc = self.gps_buffer[i]
            # Bicycle model uses a different coordinate system
            loc_temp = np.array([loc[1], -loc[0]])
            next_loc_tmp, _, _ = self.ego_model.forward(
                loc_temp, yaw, speed, action)
    ...
    def prepare_image(self, tick_data):
        ...
        for degree in self.aug_degrees:
            ...
            rgb = torch.from_numpy(self.shift_x_scale_crop(
                image, scale=self.config.scale, crop=self.config.img_resolution, crop_shift=crop_shift)).unsqueeze(0)
            image_degrees.append(rgb.to('cuda', dtype=torch.float32))
        image = torch.cat(image_degrees, dim=0)
        return image

    def iou_bbs(self, bb1, bb2):
        a = Polygon([(bb1[0, 0], bb1[0, 1]), (bb1[1, 0], bb1[1, 1]),
                    (bb1[2, 0], bb1[2, 1]), (bb1[3, 0], bb1[3, 1])])
        b = Polygon([(bb2[0, 0], bb2[0, 1]), (bb2[1, 0], bb2[1, 1]),
                    (bb2[2, 0], bb2[2, 1]), (bb2[3, 0], bb2[3, 1])])
    ...
    def prepare_lidar(self, tick_data):
        ...
        lidar_transformed = torch.from_numpy(
            lidar_to_histogram_features(lidar_transformed)).unsqueeze(0)
        lidar_transformed_degrees = [
    ...
    def prepare_goal_location(self, tick_data):
        ...
        for degree in self.aug_degrees:
            ...
            current_target_point = (
                degree_matrix @ target_point[0].cpu().numpy().reshape(2, 1)).T

            target_point_image = draw_target_point(current_target_point[0])
            target_point_image = torch.from_numpy(target_point_image)[
                None].to('cuda', dtype=torch.float32)
        ...
        target_point = torch.cat(target_point_degrees, dim=0).to(
            'cuda', dtype=torch.float32)
...
class EgoModel():
    ...
    def forward(self, locs, yaws, spds, acts):
        ...
        beta = math.atan(self.rear_wb / (self.front_wb +
                         self.rear_wb) * math.tan(wheel))
        yaws = yaws.item()
        spds = spds.item()
